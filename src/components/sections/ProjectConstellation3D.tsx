    ...
    <group ref={nodeRef} position={[Math.cos(angle) * radius, y, Math.sin(angle) * radius]}>
      <Html center>
        <div
          ...
          className={`relative flex items-center gap-3 pl-1.5 pr-4 py-2 rounded-full border backdrop-blur-md group origin-center cursor-pointer shadow-[0_10px_30px_rgba(0,0,0,0.5)] transition-all duration-300 ${isSelected ? 'border-cyan-400 bg-[#111]/80 shadow-[0_0_20px_rgba(6,182,212,0.4)]' : 'border-white/10 bg-[#050505]/60 hover:bg-[#0a0a0a]/90 hover:border-cyan-400/50 hover:shadow-[0_0_20px_rgba(6,182,212,0.25)]'}`}
        >
          {isSelected && (
            <div className="absolute inset-0 rounded-full border border-cyan-400/50 scale-[1.1] animate-pulse pointer-events-none" />
          )}

          <div className={`w-9 h-9 shrink-0 rounded-full flex items-center justify-center border border-white/5 shadow-inner transition-colors duration-300 ${isSelected ? 'bg-cyan-500/30' : 'bg-cyan-500/20'}`}>
            <span className={`font-mono text-xs font-bold tracking-wider ${isSelected ? 'text-cyan-200' : 'text-cyan-400'}`}>
              {sequenceNum}
            </span>
          </div>

          <span className={`text-sm font-mono font-medium tracking-wide whitespace-nowrap transition-colors duration-300 ${isSelected ? 'text-white' : 'text-white/80 group-hover:text-white'}`}>
            {mainTitle}
            {signalText && (
              <>
                <span className="text-white/40 px-1.5">—</span>
...
function OrbitalNodes({ projects, onNodeClick, activeNodeId }: { projects: any[], onNodeClick: (node: any) => void, activeNodeId?: string }) {
...
export function ProjectConstellation3D() {
  ...
  return (
    <section className="relative w-full h-[calc(100vh-56px)] overflow-hidden flex items-center justify-center">
      ...
      <div className={`absolute top-12 left-6 md:top-16 md:left-12 z-20 pointer-events-none transition-opacity duration-700 ${isViewingNode ? 'opacity-0' : 'opacity-100'}`}>
        <h1 className="font-mono flex items-baseline gap-2.5 mb-2">
          ...
          <span className="text-[1.35rem] md:text-[1.65rem] font-bold text-white tracking-widest uppercase shadow-sm">PROJECTS</span>
        </h1>
      ...
      <div className={`absolute inset-0 w-full h-full z-10 transition-all duration-[1200ms] ease-[cubic-bezier(0.16,1,0.3,1)] ${isViewingNode ? 'blur-[3px] scale-95 opacity-50' : 'blur-0 scale-100 opacity-100'}`}>
        <Canvas camera={{ position: [0, 0, 8.5], fov: 45 }}>
          ...
          <OrbitalNodes projects={allProjects} onNodeClick={setSelectedNode} activeNodeId={selectedNode?.id} />
        </Canvas>
      </div>
            ...
            <div className="w-full max-w-4xl h-[75vh] bg-[#0c0c0c]/95 backdrop-blur-3xl border border-white/20 rounded-xl shadow-[0_0_100px_rgba(0,0,0,0.8)] overflow-hidden flex flex-col pointer-events-auto text-left">
              ...
              <div className="h-10 bg-[#1e1e1e] border-b border-white/10 flex items-center px-4 shrink-0 shadow-sm relative">
                ...
                <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                  <span className="text-[13px] font-mono font-medium text-white/50">
              ...
              <div className="flex-1 p-6 md:p-10 overflow-y-auto bg-black text-white/80 font-mono text-sm leading-relaxed">
                ...
                {selectedNode.link && (
                  <div className="mt-12 mb-4">
                    <a href={selectedNode.link} target="_blank" rel="noreferrer" className="text-yellow-400 hover:text-yellow-300 hover:underline transition-all font-semibold break-all border border-yellow-400/30 bg-yellow-400/10 px-4 py-2 rounded-md inline-block">
                      $&gt; open {selectedNode.link}
                    </a>
                  </div>
                )}

                <div className="mt-8 text-white/30 italic pb-8">$ awaiting command... <span className="animate-pulse">_</span></div>
              </div>
